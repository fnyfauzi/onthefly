<script lang="ts">
  ...
  import { remove, run, upload } from "../parallel.remote.js";
...
<div class="flex w-[calc(w-full-16.2rem)] m-4 justify-between">
    ...
    <form
      {...upload.enhance(async ({ form, data, submit }) => {
        ...
        try {
          ...
          const url = `${apiUrl}/user_similarity_parallel/upload`;
  ...
  <div class="flex items-center space-x-4 mr-4">
    ...
    <label for="" class="label">
      <span class="text-2xl font-semibold">Parallel Similarity</span>
        ...
        <tr class="hover:bg-base-300">
          ...
          <td class="text-sm"
            ><button
              ...
              onclick={() => {
                pngToast = `${nginxUrl}/similarity/parallel/${key}/${val[2]}`;
                isShowToast = true;
              }}>{val[3]}</button
            ></td
          >
          ...
          <td>
            <a
              target="_blank"
              href={`${baseUrl}/protected/similarity/parallel/show-2d?key=${key}&name=${val[3]}&offset=0&limit=${limit}`}
          ...
          <td>
            <a
              target="_blank"
              href={`${baseUrl}/protected/similarity/parallel/show-3d?key=${key}&name=${val[3]}&offset=0&limit=${limit}`}
              class="btn btn-sm btn-error hover:btn-primary text-white">Show</a
            >
          </td>
          <td>
            <a
              download={`${val[0]}`}
              target="_blank"
              href={`${nginxUrl}/similarity/parallel/${key}/${val[0]}`}
