<script lang="ts">
  ...
  let link: string = `${baseUrl}/protected/similarity/parallel/show-2d`;
  ...
  let linkPngs = $derived.by(() => {
    return Number(data.offset) === 0
      ? `${nginxUrl}/similarity/parallel/${data.key}/pngs`
      : `${nginxUrl}/similarity/parallel/${data.key}/pngs-next-prev`;
  });
</script>
...
<div class="flex w-[calc(w-full-16.2rem)] m-4 justify-between">
  <div class="flex items-center space-x-2">
    <a
      ...
      class={`btn btn-sm btn-error hover:btn-primary text-white ${parseInt(data.offset) === 0 ? "btn-disabled" : ""}`}
      >Prev</a
    >
  ...
  <div class="flex items-center space-x-4 mr-4">
    ...
    <label for="" class="label">
      <span class="text-2xl font-semibold">Parallel Similarity - 2D Result</span
      >
...
>
  ...
  <div class="tooltip tooltip-right">
    ...
    <div class="box">
      ...
      <img
        src={`${nginxUrl}/similarity/parallel/${data.key}/${data.png}`}
        alt={data.png}
        width="250"
  ...
  {#each data.props as prop, index}
    ...
    <div class="tooltip tooltip-right">
      ...
      <div class="box">
        ...
        />
        <div class="midname">
          {`(${Number(png.split(".")[0]) + Number(data.offset)}.png), Scores: ${Number(prop["Scores"]).toFixed(2)}`}
        </div>
      </div>
    </div>
  {/each}
