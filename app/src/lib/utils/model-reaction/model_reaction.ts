...
async function loadModel(
  ...
  const url: string = `${nginxUrl}${apiPath}/${file}`;
...
function extractModel(pdb: PDB): Object3D {
  ...
  for (let i = 0; i < geometryAtoms.attributes.position.count; i++) {
    ...
    let mesh = new Mesh(sphere, new MeshPhysicalMaterial({
      color: color,
      flatShading: true,
      metalness: 1,
      roughness: 1,
  ...
  for (let i = 0; i < geometryBonds.attributes.position.count; i += 2) {
    ...
    let mesh = new Mesh(tube, new MeshPhysicalMaterial({
      color: "white",
      flatShading: true,
      metalness: 1,
      roughness: 1,
