...
class Substructure:
    ...
    def calc_bulk(self, partitions):
        # print(partitions[0]) # ['COC(=O)CCSc1nncs1', 'CHEMBL4573215', 'None', 357328]
        #
        ...
        #     for v in ex.map(self.chunk, partitions):
        #         # if v == -1: continue
        #         lst.append(v)
        # return lst
    ...
    async def run(self):
        ...
        if len_smiles < 1200000:
            ...
            async with aPool(split) as pool:
                # async for result in pool.starmap(self.make_pdb, mols):
        ...
        if not len_indices:
            ...
            # for file in files:
            #     _, ext = op.splitext(file)
            #     if ext == ".csv":
            #         os.remove(op.join(path, file))
            ...
            if op.exists(pdbs_dir):
                shutil.rmtree(pdbs_dir, ignore_errors=True)
            #
            lib_txt = op.join(path, 'lib.txt')
            if op.exists(lib_txt):
                os.remove(lib_txt)
        ...
        if is_need_pdbs:
            # pool = Pool(split)
            # pool.starmap_async(self.make_pdb, mols)
            # pool.close()
            # pool.join()
            async with aPool(split) as pool:
...
#! python parallel_substructure.py --is-show-time-open-file --is-show-time-logic --cpu 100 --rank 1000 --smile "CCCOc1ccc(OC(=O)c2cccc([N+](=O)[O-])c2)cc1" --lib /share1-100G/fanny/storage/api/app/assets/similarity/libraries/CHEMBL29_part1-364620.smi --save-to /share1-100G/fanny/storage/api/app/assets/substructure/parallel/20251126_110218_1/example-1.csv
#! python parallel_substructure.py --is-need-pngs --is-need-pdbs --cpu 100 --rank 1000 --smile "CCCOc1ccc(OC(=O)c2cccc([N+](=O)[O-])c2)cc1" --lib /share1-100G/fanny/storage/api/app/assets/similarity/libraries/CHEMBL29_part1-364620.smi --save-to /share1-100G/fanny/storage/api/app/assets/substructure/parallel/20251126_110218_1/example-1.csv

# different smiles with above
#! python parallel_substructure.py --is-show-time-open-file --is-show-time-logic --cpu 100 --rank 1000 --smile "c1ccccc1" --lib /share1-100G/fanny/storage/api/app/assets/similarity/libraries/CHEMBL29_part1-364620.smi --save-to /share1-100G/fanny/storage/api/app/assets/substructure/parallel/20251126_110218_1/example-1.csv
#! python parallel_substructure.py --is-need-pngs --is-need-pdbs --is-show-time-open-file --is-show-time-logic --cpu 100 --rank 1000 --smile "c1ccccc1" --lib /share1-100G/fanny/storage/api/app/assets/similarity/libraries/ZINC_part19-19645000.smi-[500000].smi --save-to /share1-100G/fanny/storage/api/app/assets/substructure/parallel/20251126_110218_1/example-1.csv
